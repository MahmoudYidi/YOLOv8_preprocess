...
# Hyperparameters
input_dim = 398 #448  # Number of spectral bands
...
alpha = 0.001  # Sparsity loss weight
beta = 1  # Diversity loss weight
...
pixel_data_smoothed = pixel_data 
...
def loss_function(reconstructed, original, band_weights, selected_bands, alpha=0.1, beta=10, gamm=1):
    ...
    def diversity_loss(selected_bands, min_distance=3):
        # Ensure selected_bands is a floating-point tensor
        if selected_bands.dtype != torch.float32:
            selected_bands = selected_bands.float()  # Convert to float32
        
        if selected_bands.dim() == 2:
            selected_bands = selected_bands.unsqueeze(-1)  # Add latent_dim dimension if missing
        
        batch_size, num_selected_bands, latent_dim = selected_bands.shape
        
        # Compute pairwise distances
        selected_bands = selected_bands.unsqueeze(2)  # (batch, num_selected, 1, latent_dim)
        pairwise_distances = torch.norm(selected_bands - selected_bands.transpose(1, 2), dim=-1) # batch, num_selected, num_selected)
        mask = torch.triu(torch.ones_like(pairwise_distances), diagonal=1)  # Upper triangular mask
        penalty = torch.exp(-pairwise_distances / min_distance) * mask
        num_valid_pairs = mask.sum()  # Number of upper triangular entries
        loss = penalty.sum() / (num_valid_pairs + 1e-8)
